...
def lambda_return(
  ...
  dims = list(range(reward.shape.ndims)) # [0,1]
  dims = [axis] + dims[1:axis] + [0] + dims[axis + 1:] # [0] + [] + [0] + [] = [0,0]
  if axis != 0:
...
def epsilon_action_noise(action, epsilon, act_space):
  if epsilon == 0:
    return action
  # sample uniformly
  sample_action = tfd.Uniform().sample() < epsilon
  ...
  if hasattr(act_space, 'n'):
    N = act_space.n
    # sample uniformly
    action = tf.random.categorical(tf.math.log([[1/N] * N]), 1)
    return action
  else:
    uniform_dist = tfd.Uniform(low=act_space.low, high=act_space.high)
    action = uniform_dist.sample()[None]
    return action
