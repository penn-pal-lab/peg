...
PALETTES = dict(
    ...
    contrast=(
        '#0022ff', '#33aa00', '#ff0011', '#ddaa00', '#301934', '#0088aa',
...
# upper left hand.
LEGEND = dict(
    fontsize=24, numpoints=1, labelspacing=0, columnspacing=1.2,
    handlelength=1.5, handletextpad=0.5, loc='upper left')

# LEGEND = dict(
#     fontsize=22, numpoints=1, labelspacing=0, columnspacing=1.2,
#     handlelength=1.5, handletextpad=0.5, loc='lower center')
...
def load_runs(args):
  ...
  for indir in args.indir:
    ...
    for filename in filenames:
      print(filename)
      print(filename.relative_to(indir).parts[:-1])
  ...
  print("load runs glob took ", time() - start)
  ...
  print("getting jobs took ", time() - start)
  return runs
...
def load_run(filename, indir, args):
  ...
  try:
    if False:
    # if 'goal_all' in args.yaxis:
      # Hack for getting average goals
      goal_cols = [f'mean_eval_metric_success/goal_{i}' for i in range(6, 8)]
      df['mean_eval_metric_success/goal_all'] = df[goal_cols].mean(axis=1)
      all_cols = [args.xaxis, 'mean_eval_metric_success/goal_all']
      df = df[all_cols].dropna()
    else:
      df = df[[args.xaxis, args.yaxis]].dropna()
      if args.smooth_factor > 0:
        ts_factor = args.smooth_factor
        smooth = df.ewm(alpha=(1 - ts_factor)).mean()
        df[args.yaxis] = smooth[args.yaxis]
...
def curve_area(index, task, method, ax, runs, args):
  ...
  with warnings.catch_warnings():  # NaN buckets remain NaN.
    ...
    elif args.agg == 'stderr1':
      mean, std = np.nanmean(ys, -1), np.nanstd(ys, -1)
      lo, mi, hi = mean - (std/np.sqrt(len(runs))), mean, mean + (std/np.sqrt(len(runs)))
    elif args.agg == 'per0':
...
def legend(fig, mapping=None, **kwargs):
  ...
  for ax in fig.axes:
    ...
    ax.legend(entries.values(), entries.keys(), **kwargs)
  fig.tight_layout()
...
def load_saved_runs(args):
  methods = {
    "peg": { 
      "folder":"/home/edward/logdir/stack3_mppi10_s",
      "seeds": range(0,10),
    },
    "mega": { 
      "folder":"/home/edward/logdir/stack3_mega_s",
      "seeds": range(0,10),
    },
    "skewfit": { 
      "folder":"/home/edward/logdir/stack3_skewfit_s",
      "seeds": range(0,10),
    },
  }
  bins = args.bins
  ...
  for name, metadata in methods.items():
    ...
    for seed in metadata["seeds"]:
      data_path = os.path.join(metadata["folder"] + str(seed), "3stack_eval_data.pkl")
      ...
      for r in results:
        step = r[0] 
        goal_success_dict = r[1] # {goal_idx: success}
        # for now, just use the average success.
        y_value = np.mean(list(goal_success_dict.values()))
        # y_value = goal_success_dict[14]
        xs.append(step)
        ys.append(y_value)
      xs = np.array(xs)
      ys = np.array(ys)
      if bins:
        borders = np.arange(0, xs.max() + 1e-8, bins)
        xs, ys = bin_scores(xs, ys, borders)
      r = Run("stack", name, str(seed), xs, ys)
      all_runs.append(r)
    # TODO: consider padding shorter runs with their last value 
  return all_runs 


def main(args):
  # find_keys(args)
  runs = load_saved_runs(args)
