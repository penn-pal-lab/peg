...
class EnsembleRSSM(common.Module):
  ...
  def obs_step(self, prev_state, prev_action, embed, is_first, sample=True):
    # if is_first.any():
    # zero out state and actions on batch dims where is_first
    is_first = tf.expand_dims(is_first,-1)
    def zero_if_first(x):
      return (1.0 - tf.cast(is_first,x.dtype)) * x
    prev_action = zero_if_first(prev_action)
    prev_state = tf.nest.map_structure(zero_if_first, prev_state)

    # import ipdb; ipdb.set_trace()
    # set all prev_state, prev_action who is first state to 0s.
  ...
  def lexa_kl_loss(self, post, prior, balance, free, scale):
    kld = tfd.kl_divergence
...
class Encoder(common.Module):
  ...
  def __init__(
    ...
    cnn_embed_size = (32 * cnn_depth) * (len(self.cnn_keys) > 0)
    mlp_embed_size = mlp_layers[-1] * (len(self.mlp_keys) > 0)
    self.embed_size = cnn_embed_size + mlp_embed_size
    print('Encoder embed size:', self.embed_size)
...
class GC_Distance(common.Module):
  def __init__(
      self, act='elu', layers = 4, units = 128, out_dim=1, input_type='feat', normalize_input=False):
...
class DistLayer(common.Module):
  ...
  def __call__(self, inputs):
    ...
    if self._dist in ('normal', 'tanh_normal', 'trunc_normal'):
      ...
      # try:
      #   tf.debugging.check_numerics(std, message='Checking std')
      # except:
      #   import ipdb; ipdb.set_trace()
